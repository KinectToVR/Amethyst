...
public class SetupPlugin : INotifyPropertyChanged
{
    ...
    public async Task PerformFixApplication(IFix fix, object param = null)
    {
        ...
        try
        {
            ...
            var result = true; // Treat "not supported" the same as "success"
            if (!FileUtils.IsCurrentProcessElevated() && Guid is "K2VRTEAM-AME2-APII-DVCE-DVCEKINECTV1")
            {
                Logger.Warn("Amethyst is not elevated, but the fix will likely " +
                            "require administrative privileges! Just so you know...");
            }
            else
            {
                // Capture the installation thread
                ...
                // Actually start the installation now
                result = await InstallHandler.InstallationWorker;
            }
